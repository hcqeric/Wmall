<template>
  <div class="container">
    <mt-header fixed title="商品详情">
      ...
      <mt-button slot="right" @click="toPay">去结算</mt-button>
    </mt-header>
...
<script>
  ...
  import {setSessionStorage, getLocalStorage} from "../../custom/mixin";
  ...
  export default {
    ...
    methods: {
      ...
      addToShopCart(){
        this.saveInfo.goodsId = this.goodsInfo.id
        this.saveInfo.goodsNo += 1
        setSessionStorage("goodsInfo", this.saveInfo)
    ...
    mounted(){
      let {goodsNum, userId} = this.$route.params
      this.id = goodsNum
